...
import { fetchEventById, fetchEventApply } from '@services/public/EventService'
...
const SchlorshipDetails: React.FC = () => {
        ...
        if (!program_id) {
            ...
            fetchEventById(program_id)
                .then((response) => {
                    ...
                    setHasApplied(response?.is_assigned === 1)
                    setLoading(false)
                })
                .catch((error) => {
                    setError(error)
    ...
    const onapply = async () => {
        ...
        try {
            await fetchEventApply(program_id)
            setHasApplied(true)
            enqueueSnackbar('Interest Shown!', {
                variant: 'success',
            })
        } catch (error) {
            enqueueSnackbar('Failed to apply. Please try again.', {
                variant: 'error',
            })
        }
    }
                    ...
                    <div className="md:flex">
                        <div>
                            <img
                                ...
                                alt="logo"
                                className="md:rounded-s-lg rounded-t-lg md:rounded-r-none"
                            />
                        </div>
                        <div className="w-full px-5 py-3">
                            <div className="flex justify-between items-center gap-3 border-b py-4 mb-3 w-full">
                                <h2 className="text-2xl md:text-4xl font-bold text-primary dark:text-primary w-full">
                                    {
                                        eventdetails?.competitions_details
                                            ?.program.name
                                    }
                                </h2>
                                <div>
                            ...
                            <div
                                className="text-gray-700 dark:text-gray-400 line-clamp-6"
                            ...
                            {/* <Link to={`/schlorship-activity/${program_id}`} className="bg-primary text-white dark:text-gray-900 font-bold py-2 px-6 rounded hover:bg-primary-light dark:hover:bg-primary-dark transition">
                                Join Now
                            </Link> */}
                            <button 
                                onClick={onapply} 
                                disabled={hasApplied} 
                                className={`${
                                    hasApplied ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-light dark:hover:bg-primary-dark'
                                } text-white dark:text-gray-900 font-bold py-2 px-6 rounded transition`}
                            >
                                {hasApplied ? 'Interested' : 'Show Interest'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="container mx-auto mt-72 md:mt-28 w-[95%]">
                ...
                <div className="mt-8">
                    ...
                    <div
                        ...
                        dangerouslySetInnerHTML={{
                            __html: eventdetails?.competitions_details?.program
                                .description,
                        }}
                    ></div>
                    {/* <h2 className="text-lg font-bold text-primary dark:text-primary mb-3">
                       Schlorship Activities
                    </h2>
...
export default SchlorshipDetails
